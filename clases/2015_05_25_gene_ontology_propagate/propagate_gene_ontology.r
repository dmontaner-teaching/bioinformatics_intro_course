...
##' See video [here](http://youtu.be/6EG75HboMSA).
##' 
...
##' Keep just the interesting columns (gene, GO term and ontology)
datos[1:3,]
datos <- datos[,c(3, 5, 9)]
dim (datos)
datos[1:3,]
...
##' Now we can get the pot in PNG format
pp <- getAmigoTree (goIDs = goIDs, color = color, filename = "datos/propagated_gos")
